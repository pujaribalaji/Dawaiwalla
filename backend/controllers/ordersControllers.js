...
export const getOrdersController = async (req, res) => {
  ...
  try {
    if (req.query.userId) {
      ...
      const orders = await Order.find({ user: req.query.userId }).select(
        "items"
      );
...
// import Order from '../models/Order'; // Assuming Order model is imported
...
export const updateOrderStatusController = async (req, res) => {
  ...
  try {
    ...
    // Check if status field exists, if not, create it
    if (!order.status) {
      order.status = status;
    } else {
